...
if(isset($_SESSION['loggedin']) && isset($_SESSION['id'])){
	...
	if(isset($_POST['exhibition_id']) && isset($_POST['exhibition_title']))
	{
		...
		if($stmt = $conn->prepare('UPDATE exhibitions SET exhibition_title=?, exhibition_description=?, institution_id=?, exhibition_map=?, exhibition_ip=?, exhibition_port=?, server_name=?, active=? WHERE exhibition_id=?')){
			$stmt->bind_param('ssissssii', $exhibition_title, $exhibition_description, $institution_id, $exhibition_map, $exhibition_ip, $exhibition_port, $server_name, $active, $exhibition_id);
			$stmt->execute();
