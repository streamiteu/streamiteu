...
if(isset($_SESSION['loggedin']) && isset($_SESSION['id'])){
	...
	if(isset($_POST['exhibit_name']) && isset($_POST['exhibition_id']))
	{
		...
		$exhibit_heading_w = $_POST['exhibit_heading_w'];
		$active = 1;
		try{
		if($stmt = $conn->prepare('INSERT INTO exhibits (exhibit_name,exhibit_description, exhibition_id, exhibit_location_x, exhibit_location_y, exhibit_location_z, exhibit_heading_x, exhibit_heading_y, exhibit_heading_z, exhibit_heading_w ,active) VALUES (?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?)')){
			$stmt->bind_param('ssidddddddi', $exhibit_name,$exhibit_description, $exhibition_id, $exhibit_location_x, $exhibit_location_y, $exhibit_location_z, $exhibit_heading_x, $exhibit_heading_y, $exhibit_heading_z, $exhibit_heading_w ,$active);
			$stmt->execute();
			$c_id = $stmt->insert_id;
			if ($c_id != 0) {
