...
?>
					<div class="row">
						<div class="col-md-12">
							<form id="user-insert-form" method="post">
								<section class="panel">
									...
									<div class="panel-body">
										<div class="form-group">
											...
											<div class="col-sm-9">
												<input type="text" name="firstname" class="form-control" value="" placeholder="eg.: John" required/>
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">Last Name <span class="required">*</span></label>
											<div class="col-sm-9">
												<input type="text" name="lastname" class="form-control" value="" placeholder="eg.: Doe" required/>
											</div>
										</div>
										<div class="form-group">
											...
											<div class="col-sm-9">
												<select id="user_type" class="form-control" required>
													...
													if($stmt = $conn->prepare($sql_query)){
														...
														while ($stmt->fetch()) {
													?>
													<option value="<?php echo $user_type_id ?>"><?php echo $user_type_description ?></option>
													<?php
														}
														$stmt->close();
													}
											...
											<div class="col-sm-9">
												<select id="institution-admin" class="form-control" required>
													...
													if($session_user_type_id == 2 || $session_user_type_id == 3){
														$sql_query = 'SELECT institution_id, institution_type, institution_name FROM institutions WHERE institution_id IN (SELECT institution_id FROM userinstitution WHERE userinstitution.user_id = '. $id .')';
													}
													if($session_user_type_id == 1){
													...
													if($stmt = $conn->prepare($sql_query)){
														...
														while ($stmt->fetch()) {
													?>
													<option data-institution-type="<?php echo $institution_type ?>" value="<?php echo $institution_id?>"><?php echo $institution_name?></option>
													<?php
														}
														$stmt->close();
													}
										...
										<div class="form-group">
											...
											<div class="col-sm-9">
												<input type="checkbox" id="approved" name="approved" checked value="">
											</div>
										</div>
									</div>
