<?php
	...
	if(isset($_SESSION['loggedin']) && isset($_SESSION['id'])){
		...
		if(isset($_POST['id'])){
			...
			if($stmt = $conn->prepare("SELECT tours.tour_id, tours.tour_attendees, tours.tour_exhibits, schedule.schedule_id, schedule.schedule_type, schedule.exhibition_id, schedule.schedule_start, schedule.schedule_end, institutions.institution_id, institutions.institution_type, institutions.institution_name FROM exhibitions, schedule, tours, institutions WHERE institutions.institution_id = exhibitions.institution_id AND schedule.exhibition_id = exhibitions.exhibition_id AND tours.schedule_id = schedule.schedule_id AND tours.tour_id=?")){
				$stmt->bind_param('i', $tour_id);
				$stmt->execute();
				$stmt->store_result();
				$stmt->bind_result($tour_id, $tour_attendees, $tour_exhibits, $u_schedule_id, $schedule_type, $u_exhibition_id, $schedule_start, $schedule_end, $u_institution_id, $institution_type, $institution_name);
				$stmt->fetch();
			}else{
				//echo $_POST['id'];
...
<style>
  ...
  #calendar {
								...
								<section class="panel">
									...
									<div class="panel-body">
											...
											<div class="col-sm-10">
												<select id="museum-route" class="form-control">
													...
													if($stmt = $conn->prepare("SELECT institution_id, institution_name FROM institutions WHERE institution_type='museum'")){
														...
														while ($stmt->fetch()) {
													?>
													<option <?php if ($u_institution_id == $institution_id) echo 'selected' ?> value="<?php echo $institution_id?>"><?php echo $exhibition_name?></option>
													<?php
														}
													}
													$stmt->close();
											...
											<div class="col-sm-10">
												<select id="exhibition-route" class="form-control" required>
													...
													if($stmt = $conn->prepare("SELECT exhibition_id, institution_id, exhibition_title FROM exhibitions")){
														...
														while ($stmt->fetch()) {
													?>
													<option <?php if ($u_exhibition_id == $exhibition_id) echo 'selected' ?> data-institution-id="<?php echo $institution_id ?>" value="<?php echo $exhibition_id?>"><?php echo $exhibition_title?></option>
													<?php
														}
													}
													$stmt->close();
										...
										<div class="form-group">
											...
											<div class="col-sm-12">
												<div id='calendar'></div>
												<input type="hidden" id="schedule_id" name="schedule_id" value="<?php echo $u_schedule_id?>">
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-2 control-label">School </label>
											<div class="col-sm-10">
												<select id="school-student-import" class="form-control">
										...
										<div class="form-group">
											...
											<div class="col-sm-10">
												<select id="class-student-import" class="form-control class-control">
										...
										<div class="form-group">
											...
											<div class="col-sm-4">
												...
												<?php
												if($stmt = $conn->prepare('SELECT user_id, first_name, last_name FROM users WHERE user_id IN ('.$tour_attendees.')')){
													$stmt->execute();
													$stmt->store_result();
	...
	var currentDate = new Date('<?php echo $schedule_start?>');
